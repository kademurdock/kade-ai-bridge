...
function planPrompt(goal) {
  return [
    ...
    '',
    `Hard limits: at most ${MAX_STEPS} steps, at most 3 research steps.`,
    'Do not invent facts. Do not plan phone calls, emails, purchases, or documents — you cannot do those.',
    '',
...
function spokenSummary(errand) {
  ...
  const calls = s.filter((x) => x.kind === 'call_business').length;
...
async function runErrand(errand, deps) {
  ...
  try {
    ...
    if (!errand.plan) {
      ...
      const steps = raw
        .filter((x) => x && (x.kind === 'research' || x.kind === 'confirm'))
        .slice(0, MAX_STEPS);
    ...
    while (errand.cursor < errand.plan.length) {
      ...
      if (step.kind === 'confirm') {
        errand.pending = { ask: String(step.ask || 'Should I keep going?').slice(0, 300), since: new Date().toISOString(), cursor: errand.cursor };
...
async function wrapUp(errand, deps) {
  const findings = errand.steps
    ...
    .map((x, i) => `FINDING ${i + 1} (${x.detail.question}):\n${x.detail.report}`)
    .join('\n\n');
...
function attachErrands(app, deps = {}) {
  ...
  app.post('/errand/:id/confirm', express.json({ limit: '8kb' }), (req, res) => {
    ...
    if (yes && age > CONFIRM_TTL_MS) {
      addStep(e, 'note', 'That yes came in too long after I asked, so I did not act on it. Ask me again and I will start fresh.');
      setStatus(e, 'cancelled', 'the question went stale');
      e.pending = null;
      return res.json({ ok: true, ...errandPublic(e), spokenSummary: spokenSummary(e) });
    }

    addStep(e, 'note', yes ? 'You said yes, so I kept going.' : 'You said no, so I stopped there.', { answer: yes ? 'yes' : 'no', ask: e.pending?.ask || null });
    e.pending = null;
    if (!yes) {
